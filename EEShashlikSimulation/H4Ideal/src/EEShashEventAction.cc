...
{  
  ...
  analysisManager->FillNtupleIColumn(7, nLayers);
  for( unsigned i=0; i<nLayers; ++i ) {
    EEShashCalorHit* actHit_i = (*actHC)[i];
    analysisManager->FillNtupleDColumn(8+i, actHit_i->GetEdep());
  }

  //  std::cout << "beam pos x = " << xBeamPos << std::endl;
   analysisManager->FillNtupleDColumn(90, xBeamPos  );
   //  std::cout << "beam pos y = " << yBeamPos << std::endl;
   analysisManager->FillNtupleDColumn(91, yBeamPos  );
